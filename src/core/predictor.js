...
class FlashCrashPredictor {
  ...
  calculateMomentum() {
    if (this.priceHistory.length < 2) {
      console.log(`🔍 Momentum: Not enough price history (${this.priceHistory.length} points)`);
    ...
    const momentum = ((currentPrice - basePrice) / basePrice) * 100;

    // Debug logging for momentum calculation
    console.log(`🔍 Momentum calc: Current=${currentPrice.toFixed(6)}, Base=${basePrice.toFixed(6)}, Result=${momentum.toFixed(3)}%`);
